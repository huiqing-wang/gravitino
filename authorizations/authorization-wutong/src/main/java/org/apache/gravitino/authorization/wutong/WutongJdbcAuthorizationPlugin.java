...
public class WutongJdbcAuthorizationPlugin extends JdbcAuthorizationPlugin {
  private static volatile WutongJdbcAuthorizationPlugin instance = null;

  private WutongJdbcAuthorizationPlugin(Map<String, String> config) {
    super(config);
  }

  public static synchronized WutongJdbcAuthorizationPlugin getInstance(Map<String, String> config) {
    if (instance == null) {
      synchronized (WutongJdbcAuthorizationPlugin.class) {
        if (instance == null) {
          instance = new WutongJdbcAuthorizationPlugin(config);
        }
      }
    }
    return instance;
